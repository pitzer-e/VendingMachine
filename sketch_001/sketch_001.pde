...
class CoinMechanism {
  ...
  CoinMechanism() {
    ...
    this.coinsInTube[1] = 10; // initial dime count is 10
    this.coinsInTube[2] = 10; // initial quarter count is 10
    this.coinsInTube[3] = 10; // initial dollar coin count is 10
...
void setup() {
  ...
  changeReturned = new SoundFile(this, "changereturned2.mp3");
  billAccepted = new SoundFile(this, "billaccepted.mp3");
  vendItem = new SoundFile(this, "venditem.mp3");
  buttonPressed = new SoundFile(this, "button.wav");
...
void updateSelection (int btn2Display) {
  ...
  if (letterSelected && numberSelected)
  {
    ...
    if(ccs.priceChangeMode) {
     ...
     currentMessage = ccs.vendItem(currentMessage, numLetterSel, numNumberSel, vendItem);
    }
    selectionMade = false;
